...
from causal_interp.greater_than import CORRUPTIONS, POSITIONS, GreaterThanDataset
...
SWEEP_JSON = RESULTS_DIR / "phase6_sweep.json"
...
ROUNDS: tuple[Round, ...] = (
    Round(
        ...
        receiver_input=None,
        position="END",
        expected="year head -> MLP 8 / MLP 9",
    ),
    Round(
        name="values of the round-0 heads at YY",
        question="which heads feed those heads' values at YY?",
        receiver_input="v",
        position="YY",
        expected="appendix upstream (a0.x)",
    ),
    Round(
        name="values of the round-1 heads at YY",
        question="which heads feed *those* heads' values at YY?",
        receiver_input="v",
        position="YY",
        expected="appendix upstream (a0.x)",
...
def run_corruption(model, corruption: str, n: int, seed: int) -> dict:
    ...
    clean_stats = ds.year_rank_stats(clean_logits)
    corrupted_stats = ds.year_rank_stats(corrupted_logits)
    print(f"  clean prob diff      {logit_baseline.clean_logit_diff:+.4f}"
          f"   (top year valid {clean_stats['top_year_is_valid']:.1%})")
    print(f"  corrupted prob diff  {logit_baseline.corrupted_logit_diff:+.4f}"
          f"   (top year valid {corrupted_stats['top_year_is_valid']:.1%})")
...
def preregister(model, n: int, seed: int) -> int:
    ...
    print(f"\n  RECALIBRATED THRESHOLD   : {threshold}   (Phase 3's was 0.11)")
    print(f"\nwrote {PREREG_JSON}")
...
def stage_main(model, n: int, seed: int) -> int:
    ...
    from phase6_report import write_report  # noqa: PLC0415

    write_report(RESULTS_DIR / "PHASE6_REPORT.md", payload)
    print(f"\nwrote {RESULTS_DIR / 'PHASE6_REPORT.md'}  ({meta['runtime_seconds']}s)")
    return 0
...
def _meta(model, n: int, seed: int, started: float) -> dict:
    return {
        ...
        "task": "greater-than (Hanna et al. 2023)",
...
def _write_sweep_csvs(corruptions: dict, out_dir: Path) -> None:
    with (out_dir / "phase6_head_effects.csv").open("w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
    ...
    with (out_dir / "phase6_component_effects.csv").open("w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        w.writerow(["corruption", "component", "layer", "position", "effect", "published_mlp"])
        for corruption, block in corruptions.items():
            for kind, rows in block["components"].items():
                for layer, per_pos in enumerate(rows):
                    published = kind == "mlp_out" and layer in gt.PUBLISHED_MLPS
                    for p, value in enumerate(per_pos):
                        w.writerow([corruption, kind, layer, POSITIONS[p],
                                    f"{value:.6f}", "yes" if published else ""])
...
def _write_search_csvs(semantic: dict, absolute: dict, out_dir: Path) -> None:
    for name, block in (("semantic", semantic), ("absolute", absolute)):
        with (out_dir / f"phase6_receiver_search_{name}.csv").open(
...
def main() -> int:
    ...
    if args.report_only:
        ...
        write_report(RESULTS_DIR / "PHASE6_REPORT.md", payload)
        print(f"rebuilt {RESULTS_DIR / 'PHASE6_REPORT.md'} from stored results (no GPU work)")
        return 0
